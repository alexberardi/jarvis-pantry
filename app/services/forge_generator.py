...
FORGE_MODELS: dict[str, ModelConfig] = {
    "haiku": ModelConfig(
        id="haiku",
...
# Approximate prompt size for cost estimation
_PROMPT_OVERHEAD_TOKENS = 7000  # ~28K chars of system prompt
_OUTPUT_TOKENS = 6000  # typical generated package


def get_available_models() -> list[dict[str, Any]]:
    """Return model list with estimated per-generation costs."""
    models = []
    for model in FORGE_MODELS.values():
        input_cost = (_PROMPT_OVERHEAD_TOKENS / 1_000_000) * model.input_price
        output_cost = (_OUTPUT_TOKENS / 1_000_000) * model.output_price
...
async def generate_package(
    ...
    Args:
        ...
        model_id: Model ID from FORGE_MODELS registry.
        conversation_history: Optional prior messages for iterative refinement.

    Returns:
        ForgeResult with generated files and explanation.
    """
    model = FORGE_MODELS.get(model_id)
    if not model:
        raise ValueError(f"Unknown model: {model_id}. Available: {', '.join(FORGE_MODELS.keys())}")
    ...
    if model.provider == "anthropic":
        raw = await _call_claude(system_prompt, messages, llm_api_key, model.api_model, model.max_tokens)
    elif model.provider == "openai" and model.api_type == "responses":
        raw = await _call_openai_responses(system_prompt, messages, llm_api_key, model.api_model, model.max_tokens)
    elif model.provider == "openai":
        raw = await _call_openai(system_prompt, messages, llm_api_key, model.api_model, model.max_tokens)
    else:
...
async def _call_claude(
    ...
    """Call Anthropic API for package generation."""
...
async def _call_openai(
    ...
    """Call OpenAI API for package generation."""
...
async def _call_openai_responses(
    ...
    # Build a single input combining system prompt + conversation
